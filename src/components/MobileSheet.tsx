...
export function MobileSheet({ children, tone, closing = false }: MobileSheetProps) {
  ...
  // Publish the sheet's actual current height (dragged or default) so
  // MapView can pad its centering to keep a selected pin inside the
  // part of the screen the sheet doesn't cover — reset to 0 on unmount
  // so a stale value doesn't linger after the sheet closes and the map
  // goes back to using the full viewport.
  //
